...
from custom_preference import CustomPreferenceComparisons
...
writer_1 = SummaryWriter(os.path.join(LOG_DIR,'GRU'))
...
rng = np.random.default_rng(0)
writer_2 = SummaryWriter(os.path.join(LOG_DIR,'Non_GRU'))
...
for index ,(model, writer) in enumerate(zip([agent, agent2],[writer_1, writer_2])):
    ...
    env = RecordVideo(env= env, 
                      video_folder= os.path.join(LOG_DIR), 
    ...
    for epi in range(TEST_EPI):
        ...
        while not is_done:
            ...
            if terminated or truncated:
                is_done = True 
            writer.add_scalar('eval/',total_reward, step)
            step += 1 
            total_reward += reward
